...
pipeline {
    ...
    parameters {
        ...
        // Added password parameter for the API key to keep it masked in the UI
        password(
            name: 'OPENWEATHER_API_KEY_PARAM',
            defaultValue: '',
            description: 'Provide your OpenWeatherMap API Key here. (Only required if RUN_LIVE_TESTS is checked).'
        )
    }
    ...
    stages {
        ...
        stage('Execute Live API Tests') {
            ...
            steps {
                // Instead of fetching from Jenkins Credentials store, 
                // we map the parameter value directly to the environment variable.
                withEnv(["OPENWEATHER_API_KEY=${params.OPENWEATHER_API_KEY_PARAM}"]) {
                    sh '''
                        echo "Running test suite against live OpenWeatherMap API..."
                        .venv/bin/pytest \
                            --live \
